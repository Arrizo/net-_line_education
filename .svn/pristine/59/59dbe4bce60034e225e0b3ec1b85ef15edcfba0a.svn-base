...
<div>
  ...
  <mt-swipe-item v-for="(item,index) in advList" :key="index">
    <img :src="item.advImg" :title="item.advTitle" @click="tolink(item.advUrl)">
  </mt-swipe-item>
</mt-swipe>
  </div>
  <marquee />
...
export default {
  ...
  methods: {
    ...
    toTest () {
      // if (this.plantInfo.msg) {
      //   this.$popBox.Toast(this.plantInfo.msg)
      //   return false
      // }
      ...
      //   this.$popBox.Toast('自评时间：' + this.plantInfo.startTime + '—' + this.plantInfo.endTime)
      //   return false
      // }  5
      this.$router.push({name: 'KeepExam', query: {type: 5}})
    },
    // 广告列
    getBannerList () {
