<template>
  ...
  <!-- <mt-tab-item id="tab1"></mt-tab-item> -->
  ...
  methods: {
    async loginSubmit () {
      ...
      userLogin(this.loginInfo).then(res => {
        if (res.code === 200) {
          this.$store.dispatch('loginInfo', res.data).then(() => {
    ...
    // 注册
    registSubmit () {
      if (!(/^(?:(?:\+|00)86)?1[3-9]\d{9}$/.test(this.registData.mobile.trim())) || this.registData.mobile.trim().length !== 11) {
