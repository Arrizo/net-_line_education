...
<div class="subject-warp">
    <div class="subject" v-for="(item,index) in testArray" :key="index+'fdd'" v-show="index===curIndex">
      ...
      {{`${index+1}、${ item.content}`}}<span class="test-type">（{{ '134'.includes(item.type)?'单选题':'多项选题'}}）</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    next () {
      ...
      this.curIndex++
      // 下一步是否组数，没有新建一个
      this.curIndex + 1 === this.testArray.length && this.getTest()
    },
    confirm (item, index) {
      if (this.answerArray[this.curIndex].length === 0) {
        this.$popBox.Toast({message: '请选择答案!', duration: 1000})
